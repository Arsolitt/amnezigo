...
| vpn:// links import into the AmneziaVPN app, NOT standalone AmneziaWG `(med)` | The `amnezigo.vpn` file from `--vpn-links` uses an AmneziaVPN-specific envelope (qCompress+zlib+base64url JSON). The standalone AmneziaWG client and kernel module cannot import these links. | `vpnlink.go` (envelope structs, `vpnContainerAWG = amnezia-awg`) | Use the AmneziaVPN mobile/desktop app to import vpn:// links. For standalone AWG, use the `awg0.conf` file directly. See [`./vpn-links.md`](./vpn-links.md). |
